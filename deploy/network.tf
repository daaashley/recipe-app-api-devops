...
resource "aws_route" "private_a_internet_out" {
  route_table_id         = aws_route_table.private_a.id
  nat_gateway_id         = aws_nat_gateway.public_a.id #Use a NAT gateway for outbound internet access only, we have to go through public network to do this
  destination_cidr_block = "0.0.0.0/0"
}
...
resource "aws_route" "private_b_internet_out" {
  route_table_id         = aws_route_table.private_b.id
  nat_gateway_id         = aws_nat_gateway.public_b.id #Use a NAT gateway for outbound internet access only, we have to go through public network to do this
  destination_cidr_block = "0.0.0.0/0"
}